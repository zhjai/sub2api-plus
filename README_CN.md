...
版本号遵循上游基线，但不会冒充上游发行版。当前 fork 版本线为 `0.2.7-zhjai.7`：`0.2.7` 表示上游基线，`-zhjai.7` 表示本 fork 的第 7 次派生修订。更新器只识别带有该派生后缀的 Release，不会把历史上不带后缀的 fork 标签当作当前版本。
...
```bash
curl -sSL https://raw.githubusercontent.com/zhjai/sub2api-plus/main/deploy/install.sh | sudo bash -s -- upgrade -v v0.2.7-zhjai.7
```
