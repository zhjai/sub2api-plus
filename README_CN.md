...
- **🚫 不构成商业背书**：本 fork 不代表或运营任何第三方商业服务。适用许可证为 LGPL-3.0-or-later，部署运营方应自行审阅该许可证及所使用上游服务商的条款。
...
版本号遵循上游基线，但不会冒充上游发行版。当前 fork 版本线为 `0.2.7-zhjai.8`：`0.2.7` 表示上游基线，`-zhjai.8` 表示本 fork 的第 8 次派生修订。更新器只识别带有该派生后缀的 Release，不会把历史上不带后缀的 fork 标签当作当前版本。

## 赞助说明

本 fork 不代表上游赞助商关系，也不对上游赞助商广告作背书。除非获得针对本仓库的独立授权，否则不在此列出商业推广内容。仓库中的合作方 logo 仅作为上游历史资源保留，不构成推荐或赞助声明。
...
```bash
curl -sSL https://raw.githubusercontent.com/zhjai/sub2api-plus/main/deploy/install.sh | sudo bash -s -- upgrade -v v0.2.7-zhjai.8
```
...
`codex.response.metadata` 只是前导事件，不代表请求完成。如果上游在该事件之后直接关闭，且没有发送 `response.completed`、`response.done`、`response.incomplete`、`response.failed` 或取消类终止事件，网关会判定为上游提前 EOF。只有在尚未向客户端发送有效文本或工具参数时才允许内部切号；已经发送正文或工具调用后会以 incomplete 结束，禁止重放整个 turn。

如果上游响应声明的模型与实际发送的模型不一致（例如请求 `gpt-6-astra`，响应却声明 `gpt-5.6-luna`），本 fork 会记录模型完整性故障，临时隔离该 OpenAI 账号，并让后续调度选择其他账号。配置中明确的合法别名在规范化后相同的情况下不会误判。
...
<a href="https://star-history.dera.page/#zhjai/sub2api-plus&Date">
 <picture>
   <source media="(prefers-color-scheme: dark)" srcset="https://star-history.dera.page/svg?repos=zhjai/sub2api-plus&type=Date&theme=dark" />
   <source media="(prefers-color-scheme: light)" srcset="https://star-history.dera.page/svg?repos=zhjai/sub2api-plus&type=Date" />
   <img alt="Star History Chart" src="https://star-history.dera.page/svg?repos=zhjai/sub2api-plus&type=Date" />
 </picture>
</a>
...
上游版权：Copyright (c) 2026 Wesley Liddick and contributors。

Fork 修改：Copyright (c) 2026 zhjai/sub2api-plus contributors。
