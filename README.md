...
Versioning follows the upstream baseline without impersonating upstream releases. The current fork line is `0.2.7-zhjai.8`: `0.2.7` identifies the upstream baseline and `-zhjai.8` identifies the eighth fork revision. The updater only considers releases with this derived suffix; legacy unqualified fork tags are not treated as current releases.
