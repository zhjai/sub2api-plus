...
- **🚫 No Commercial Endorsement**: This fork does not endorse or operate third-party commercial services. The applicable license is the LGPL-3.0-or-later license; deployment operators must review that license and the terms of any upstream providers they use.
...
Versioning follows the upstream baseline without impersonating upstream releases. The current fork line is `0.2.7-zhjai.7`: `0.2.7` identifies the upstream baseline and `-zhjai.7` identifies the seventh fork revision. The updater only considers releases with this derived suffix; legacy unqualified fork tags are not treated as current releases.
...
Upstream copyright: Copyright (c) 2026 Wesley Liddick and contributors.

Fork modifications: Copyright (c) 2026 zhjai/sub2api-plus contributors.
